...
var commands = {
  app: function (options) {
    ...
    .on('data', function(package){
      ...
      package.dependencies = disectModules(package.dependencies);
      package.devDependencies = disectModules(package.devDependencies);

      var target = package.name;

      wrench.copyDirSyncRecursive(source, target, { preserve : true });
      fs.writeFile(path.resolve(target, 'package.json'), JSON.stringify(package, null, '  '), function(err){
